...
mod tests {
    ...
    const CONTRACT_SET_ID: &str = "demo-contract-set";
    const DAO_CODE: &str = "demo-dao";
    const GOVERNOR: &str = "0x00000000000000000000000000000000000000a1";
    const TOKEN: &str = "0x00000000000000000000000000000000000000a2";
    ...
    const SECOND_DELEGATE: &str = "0x0000000000000000000000000000000000000c03";
    ...
    fn test_redelegate_marks_previous_relation_inactive_with_zero_power() {
        let repository = project_events(vec![
            delegate_changed("delegate", 1, 0, 1, ZERO_ADDRESS, DELEGATE),
            transfer("mint", 2, 0, 1, ZERO_ADDRESS, DELEGATOR, "100"),
            delegate_changed("redelegate", 3, 0, 1, DELEGATE, SECOND_DELEGATE),
        ]);

        let previous_relation = repository
            .delegates()
            .get(&delegate_ref(DELEGATOR, DELEGATE))
        ...
        let current_relation = repository
            .delegates()
            .get(&delegate_ref(DELEGATOR, SECOND_DELEGATE))
        ...
        assert_eq!(mapping.to, SECOND_DELEGATE);
    }
    ...
    fn test_power_update_preserves_delegate_mapping_relation_metadata() {
        let repository = project_events(vec![
            delegate_changed("delegate", 1, 0, 1, ZERO_ADDRESS, DELEGATE),
            transfer("mint", 2, 0, 1, ZERO_ADDRESS, DELEGATOR, "40"),
        ]);
        ...
        assert_eq!(mapping.common.block_number, "1");
    ...
    fn test_redelegated_power_update_preserves_current_relation_metadata() {
        let repository = project_events(vec![
            delegate_changed("delegate", 1, 0, 1, ZERO_ADDRESS, DELEGATE),
            transfer("mint", 1, 0, 2, ZERO_ADDRESS, DELEGATOR, "100"),
            delegate_changed("redelegate", 2, 0, 1, DELEGATE, SECOND_DELEGATE),
            delegate_votes_changed("second-votes", 2, 0, 2, SECOND_DELEGATE, "0", "100"),
            transfer("send", 3, 0, 1, DELEGATOR, RECEIVER, "25"),
        ]);
        ...
        assert_eq!(mapping.common.block_number, "2");
        assert_eq!(mapping.common.transaction_hash, "0xtx20");
    }

    fn project_events(events: Vec<TokenProjectionEvent>) -> InMemoryTokenProjectionRepository {
        let batch = project_token_events(&token_projection_context(), events)
            .expect("token projection should succeed");
        let mut repository = InMemoryTokenProjectionRepository::default();
        repository
            .apply(&batch)
            .expect("in-memory token writes should succeed");
        repository
    }
    ...
    fn delegate_changed(
        ...
        TokenProjectionEvent {
            log: normalized_log(id, block_number, transaction_index, log_index),
    ...
    fn normalized_log(
        ...
        NormalizedEvmLog {
            ...
            data: "0x".to_owned(),
            removed: false,
            raw_payload: serde_json::json!({ "block_number": block_number }),
        }
    }
}
