...
-> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(statuses[1]["provisionalHeight"], 1120);
    ...
    assert_eq!(scoped_data["indexerStatus"]["provisionalHeight"], 1120);
...
async fn seed_rows(pool: &PgPool) -> Result<(), sqlx::Error> {
    ...
    sqlx::query(
        ...
        INSERT INTO degov_provisional_segment (
          id, contract_set_id, chain_id, dao_code, dataset_key, selector, selector_fingerprint,
          range_start_block, range_end_block, segment_finality,
          source, status, anchor_block_number, anchor_block_timestamp
        ) VALUES
          ('segment:available:old', $1, 1135, 'lisk-dao', 'evm.logs', 'selector', 'selector',
           901, 1100, 'safe_to_latest', 'live-onchain', 'available', 1100, 1700000300),
          ('segment:available:new', $1, 1135, 'lisk-dao', 'evm.logs', 'selector', 'selector',
           1101, 1120, 'safe_to_latest', 'live-onchain', 'available', 1120, 1700000400),
          ('segment:rolled-back', $1, 1135, 'lisk-dao', 'evm.logs', 'selector', 'selector',
           1121, 1200, 'safe_to_latest', 'live-onchain', 'rolled_back', 1200, 1700000500)
        "#,
    )
    .bind(CONTRACT_SET_ID)
    .execute(pool)
