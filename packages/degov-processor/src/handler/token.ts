...
import { DegovConfigIndexLogContract, DgvContributor, DgvDelegate, DgvDelegateChanged, DgvDelegateMapping, DgvDelegateRolling, DgvDelegateVotesChanged, DgvTokenTransfer, EvmFieldSelection } from "../types";
...
export class TokenHandler {
  ...
  private async storeDelegateChanged(eventLog: EvmLog<EvmFieldSelection>) {
    ...
    const entity: DgvDelegateChanged = {
      id: eventLog.id,
    ...
    const currentDelegateMapping: DgvDelegateMapping = {
      id: entity.delegator,
    ...
    const delegateRolling: DgvDelegateRolling = {
      id: eventLog.id,
      delegator: event.delegator,
      fromDelegate: event.fromDelegate,
      toDelegate: event.toDelegate,
    ...
    if (
      ...
      const selfDelegate: DgvDelegate = {
  ...
  private async updateDelegateRolling(options: DgvDelegateVotesChanged) {
    const delegateRolling: DgvDelegateRolling | undefined =
  ...
  private async storeTokenTransfer(eventLog: EvmLog<EvmFieldSelection>) {
    ...
    // store delegate
    const storedFromDelegate: DgvDelegateMapping | undefined =
    ...
    const storedToDelegate: DgvDelegateMapping | undefined =
      await this.ctx.store.findOne(DelegateMapping, {
        where: {
          from: entity.to,
        },
    ...
    if (storedFromDelegate) {
      const fromDelegate: DgvDelegate = {
  ...
  private async storeDelegate(currentDelegate: DgvDelegate, options?: {}) {
    ...
    let storedDelegateFromWithTo: DgvDelegate | undefined =
    ...
    // store contributor
    const contributor: DgvContributor = {
  ...
  private async storeContributor(contributor: DgvContributor) {
    let storedContributor: DgvContributor | undefined =
