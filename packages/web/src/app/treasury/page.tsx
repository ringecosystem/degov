...
export default function Treasury() {
  ...
  const { assets, totalValueUSD, isLoading } = useTreasuryAssets();

  const hasValuedAssets = assets.some(
    (asset) => asset.balanceUSD !== null && asset.balanceUSD !== undefined
  );

  const totalValueDisplay = useMemo(() => {
    if (!hasValuedAssets) return null;
    return formatNumberForDisplay(totalValueUSD, 2)[0];
  }, [hasValuedAssets, totalValueUSD]);
    ...
    <div className="flex flex-col gap-[15px] lg:gap-[20px]">
      <header className="flex flex-col gap-[10px] sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-[5px]">
          <h3 className="text-[16px] font-semibold text-foreground lg:text-[18px]">
            Treasury Assets
          </h3>
          {formattedAddress ? (
            <span className="text-[12px] font-normal text-muted-foreground lg:text-[16px]">
              {formattedAddress}
            </span>
          ) : null}
        ...
        <div className="flex items-center gap-[10px]">
          ...
          {isLoading ? (
            ...
            <div className="flex items-center gap-[10px] text-[20px] font-semibold leading-normal text-foreground lg:text-[26px]">
              {totalValueDisplay} USD
            </div>
          ) : (
            <div className="flex items-center gap-[10px] text-[20px] font-semibold leading-normal lg:text-[26px]">
              N/A
              <Tooltip>
                <TooltipTrigger>
      ...
      <div className="space-y-6 lg:hidden">
        <TreasuryList data={assets} isLoading={isLoading} />
        <SafeList />
      </div>

      <div className="hidden space-y-[20px] lg:block">
        <TreasuryTable data={assets} isLoading={isLoading} />
